    ...
    companion object {
        ...
        private fun cleanAirbyteInternalTable(database: JdbcDatabase?) {
            ...
            if (!cleanedAirbyteInternalTable.getAndSet(true)) {
                ...
                executor.execute {
                    database!!.execute(
                        "DELETE FROM \"airbyte_internal\".\"_airbyte_destination_state\" WHERE \"updated_at\" < timestampadd('hours', -$cleanupCutofHours, current_timestamp())",
                    )
                }
                executor.execute {
                    database!!.execute(
                ...
                executor.execute {
                    ...
                    schemaList
                        ...
                        .filter { it.value.isBefore(Instant.now().minus(6, ChronoUnit.HOURS)) }
                        .filter {
